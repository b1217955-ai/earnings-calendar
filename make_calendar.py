...
def add_fallback_important_events(events, start_date, end_date):
    """公式日程で確認した主要経済指標・大型IPO。SBIが取れない時でも重要事項を消さない。"""
    fallback = [
        ...
        ('2026-06-10', 'jp', '企業物価指数（前年比）-5月', '8:50'),
        ('2026-06-10', 'jp', '企業物価指数（前月比）-5月', '8:50'),
        ('2026-06-10', 'us', '⭐ 消費者物価指数[コア]（前年比）-5月', '21:30'),
        ('2026-06-10', 'us', '⭐ 消費者物価指数[コア]（前月比）-5月', '21:30'),
        ('2026-06-10', 'us', '⭐ 消費者物価指数（前年比）-5月', '21:30'),
        ('2026-06-10', 'us', '⭐ 消費者物価指数（前月比）-5月', '21:30'),
        ('2026-06-11', 'jp', '法人企業景気予測調査-2Q', '8:50'),
        ('2026-06-11', 'us', '⭐ 生産者物価指数[コア]（前年比）-5月', '21:30'),
        ('2026-06-11', 'us', '⭐ 生産者物価指数[コア]（前月比）-5月', '21:30'),
        ('2026-06-11', 'us', '⭐ 生産者物価指数（前年比）-5月', '21:30'),
        ('2026-06-11', 'us', '⭐ 生産者物価指数（前月比）-5月', '21:30'),
        ('2026-06-11', 'us', '新規失業保険申請件数-6/6', '21:30'),
        ('2026-06-12', 'jp', '⭐ メジャーSQ（株価指数先物・オプション特別清算指数）', '9:00'),
        ('2026-06-12', 'us', '⭐ 米国大型IPO上場予定：SpaceX（SPCX・Nasdaq）', '22:30'),
        ('2026-06-12', 'us', 'ミシガン大学消費者信頼感指数（速報）-6月', '23:00'),
        ('2026-06-16', 'jp', '⭐ 日銀金融政策決定会合 結果発表・総裁会見', '15:30'),
        ('2026-06-16', 'us', '住宅着工件数-5月', '21:30'),
        ('2026-06-17', 'jp', '機械受注（前月比）-4月', '8:50'),
        ('2026-06-17', 'jp', '貿易収支-5月', '8:50'),
        ('2026-06-17', 'us', '⭐ 小売売上高[除自動車]（前月比）-5月', '21:30'),
        ('2026-06-17', 'us', '⭐ 小売売上高（前月比）-5月', '21:30'),
        ('2026-06-18', 'us', '⭐ FOMC政策金利・声明発表（議長会見 3:30）', '3:00'),
        ('2026-06-19', 'jp', '⭐ 全国消費者物価指数[除生鮮食品]（前年比）-5月', '8:30'),
        ('2026-06-19', 'jp', '⭐ 全国消費者物価指数（前年比）-5月', '8:30'),
        ('2026-06-19', 'jp', '⭐ 日銀金融政策決定会合議事要旨（4月27-28日分）', '8:50'),
        ('2026-06-24', 'jp', '⭐ 日銀金融政策決定会合 主な意見（6月15-16日分）', '8:50'),
        ('2026-06-25', 'us', '⭐ 個人消費支出[コアPCEデフレーター]-5月', '21:30'),
        ('2026-06-25', 'us', '⭐ 個人消費支出・PCEデフレーター-5月', '21:30'),
        ('2026-06-25', 'us', '⭐ 実質GDP（確報値・前期比年率）-1Q', '21:30'),
        ('2026-06-26', 'jp', '⭐ 東京都区部消費者物価指数（前年比）-6月', '8:30'),
        ('2026-06-30', 'jp', '⭐ 完全失業率-5月', '8:30'),
        ('2026-06-30', 'jp', '有効求人倍率-5月', '8:30'),
        ('2026-06-30', 'jp', '鉱工業生産（速報・前月比）-5月', '8:50'),
        ('2026-06-30', 'us', 'CB消費者信頼感指数-6月', '23:00'),
        ('2026-07-01', 'jp', '⭐ 日銀短観[大企業製造業業況判断]-2Q', '8:50'),
        ('2026-07-01', 'jp', '⭐ 日銀短観[大企業非製造業業況判断]-2Q', '8:50'),
        ('2026-07-01', 'jp', '⭐ 大型IPO上場日：ギフティグループ（590A・東証プライム）', '9:00'),
        ...
        ('2026-07-07', 'jp', '毎月勤労統計[現金給与総額]（前年比）-5月', '8:30'),
        ('2026-07-07', 'jp', '景気動向指数[一致指数]（速報）-5月', '14:00'),
        ('2026-07-07', 'jp', '景気動向指数[先行指数]（速報）-5月', '14:00'),
        ('2026-07-07', 'us', '貿易収支-5月', '21:30'),
        ('2026-07-08', 'jp', '国際収支[経常収支]-5月', '8:50'),
        ('2026-07-08', 'jp', '国際収支[貿易収支]-5月', '8:50'),
        ('2026-07-09', 'us', '⭐ FOMC議事要旨（6月16-17日分）', '3:00'),
        ('2026-07-10', 'jp', '⭐ SQ（株価指数オプション特別清算指数）', '9:00'),
        ('2026-07-14', 'us', '⭐ 消費者物価指数[コア]（前年比）-6月', '21:30'),
        ('2026-07-14', 'us', '⭐ 消費者物価指数[コア]（前月比）-6月', '21:30'),
        ('2026-07-14', 'us', '⭐ 消費者物価指数（前年比）-6月', '21:30'),
        ('2026-07-14', 'us', '⭐ 消費者物価指数（前月比）-6月', '21:30'),
        ('2026-07-15', 'us', '⭐ 生産者物価指数[コア]（前年比）-6月', '21:30'),
        ('2026-07-15', 'us', '⭐ 生産者物価指数[コア]（前月比）-6月', '21:30'),
        ('2026-07-15', 'us', '⭐ 生産者物価指数（前年比）-6月', '21:30'),
        ('2026-07-15', 'us', '⭐ 生産者物価指数（前月比）-6月', '21:30'),
        ('2026-07-16', 'us', '⭐ 小売売上高[除自動車]（前月比）-6月', '21:30'),
        ('2026-07-16', 'us', '⭐ 小売売上高（前月比）-6月', '21:30'),
        ('2026-07-17', 'jp', '⭐ 全国消費者物価指数[除生鮮食品]（前年比）-6月', '8:30'),
        ('2026-07-17', 'jp', '⭐ 全国消費者物価指数（前年比）-6月', '8:30'),
        ('2026-07-30', 'us', '⭐ FOMC政策金利・声明発表（議長会見 3:30）', '3:00'),
        ('2026-07-30', 'us', '⭐ 個人消費支出[コアPCEデフレーター]-6月', '21:30'),
        ('2026-07-30', 'us', '⭐ 個人消費支出・PCEデフレーター-6月', '21:30'),
        ('2026-07-30', 'us', '⭐ 実質GDP（速報値・前期比年率）-2Q', '21:30'),
        ('2026-07-31', 'jp', '⭐ 東京都区部消費者物価指数（前年比）-7月', '8:30'),
        ('2026-07-31', 'jp', '⭐ 日銀金融政策決定会合 結果発表・展望レポート', '15:30'),
    ]
    ...
    for ds, country, name, time_txt in fallback:
        ...
        events.setdefault(dt, []).append({
            ...
            "time": time_txt,
            "importance": 3,
            "commentary": economic_commentary(name, country_name),
        })
    for items in events.values():
        items.sort(key=lambda x: (time_sort_value(x.get("time", "")), x.get("country", ""), x.get("name", "")))
    return events
