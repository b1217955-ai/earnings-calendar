...
def extract_tse33_sectors() -> list[str]:
    source = read(SECTOR_SCRIPT)
    tree = ast.parse(source)
    for node in tree.body:
        if isinstance(node, ast.Assign):
            for target in node.targets:
                if isinstance(target, ast.Name) and target.id == "TSE33_SECTORS":
...
def build_context() -> str:
    ...
    sector_lines = []
    items = list(enumerate(sectors, 1))
    for i in range(0, len(items), 3):
        chunk = items[i:i + 3]
        sector_lines.append("    ".join(f"{num:02d}. {name}" for num, name in chunk))
...
Mac側のPTS同期は、毎回次のiCloud Driveフォルダへ `.txt` と `.png` を保存する。

```txt
/Users/hamaguchitomohiro/Library/Mobile Documents/com~apple~CloudDocs/AI Codex/自動化出力/PTS
```
